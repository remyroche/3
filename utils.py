...
from .database import get_db_connection, query_db # Added query_db
...
# --- NEW: Static JSON Generation for Build Process ---
def generate_static_json_files():
    """
    Fetches all active products and categories from the database and writes them
    to JSON files used by the static site build process.
    """
    if not current_app:
    ...
    with current_app.app_context():
        ...
        # --- Generate products_details.json ---
        try:
            ...
            # For each product, fetch its variants and other images
            for product in products_list:
                ...
                # Fetch and add variants if it's a variable product
                if product['type'] == 'variable_weight':
                    options_data = query_db(
                        """SELECT weight_grams, price, sku_suffix, aggregate_stock_quantity as stock_quantity, is_active
                    ...
                    product['stock_quantity'] = product.get('aggregate_stock_quantity', 0)


                # Fetch and add additional images
            ...
            # Define output path and write the file
        ...
        # --- Generate categories_details.json ---
        try:
            categories_data = query_db(
                "SELECT id, name, slug, description, image_url, parent_id FROM categories WHERE is_active = TRUE ORDER BY name",
                db_conn=db
            )
