...
function initializeProductManagement() {
    ...
    // Check for URL action to open form
...
/**
 * Opens and prepares the product form for adding a new product.
 */
function openProductFormForAdd() {
    ...
    if (!productFormSection || !showFormButton || !productForm) return;
...
async function loadAdminProductsList() {
    ...
    try {
        productsForAdmin = await adminApiRequest('/api/products'); 
...
async function editProduct(productId) {
    ...
    try {
        productToEdit = await adminApiRequest(`/api/products/${productId}`); 
