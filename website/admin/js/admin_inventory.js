        ...
        if (select) {
            select.addEventListener('change', (event) => {
                if (event.target.id !== 'inventory-filter-product') {
                    populateVariantSelect(event.target.value, event.target.id);
                } else {
...
/**
 * Populates or hides the variant selection dropdown based on the selected product.
 * @param {string} selectedProductId - The ID of the selected product.
 * @param {string} productSelectElementId - The ID of the product select dropdown that triggered the change.
 */
async function populateVariantSelect(selectedProductId, productSelectElementId) {
    let variantSelectContainerId, variantSelectId;

    // Determine which variant dropdown to update based on the product select ID
    if (productSelectElementId === 'stock-product-select') {
        variantSelectContainerId = 'stock-variant-select-container';
        variantSelectId = 'stock-variant-select';
    } else if (productSelectElementId === 'adjust-product-select') {
    ...
    resetVariantSelect(variantContainer, variantSelect); // Reset first
...
/**
 * Resets a variant select dropdown to its initial state.
 * @param {HTMLElement} container - The container of the variant select.
 * @param {HTMLSelectElement} select - The variant select element.
 */
function resetVariantSelect(container, select) {
...
async function handleAddStockSubmit(event) {
    ...
    const payload = getInventoryAdjustmentPayload(form, 'add');
    if (!payload) return; // Should not happen if validation passes
    ...
    try {
        ...
        if (result.success) {
            ...
            form.reset();
            resetVariantSelect(document.getElementById('stock-variant-select-container'), document.getElementById('stock-variant-select'));
...
async function handleManualAdjustSubmit(event) {
    ...
    const payload = getInventoryAdjustmentPayload(form, 'adjust');
    if (!payload) return; // Should not happen if validation passes
    ...
    try {
        ...
        if (result.success) {
            ...
            form.reset();
            resetVariantSelect(document.getElementById('adjust-variant-select-container'), document.getElementById('adjust-variant-select'));
            loadInventoryOverview(); // Refresh inventory table
        }
    } catch (error) {
...
/**
 * Constructs the payload for an inventory adjustment API call.
 * @param {HTMLFormElement} form - The form element.
 * @param {string} type - 'add' or 'adjust'.
 * @returns {object|null} The payload object or null if essential fields are missing.
 */
function getInventoryAdjustmentPayload(form, type) {
    const productIdField = form.elements[type === 'add' ? 'product_id' : 'product_id_adjust'];
    const variantOptionIdField = form.elements[type === 'add' ? 'variant_option_id' : 'variant_option_id_adjust'];
    const quantityField = form.elements[type === 'add' ? 'quantity_add' : 'quantity_change'];
    const movementTypeField = form.elements[type === 'add' ? 'movement_type_add' : 'movement_type_adjust'];
    const notesField = form.elements[type === 'add' ? 'notes_add' : 'notes_adjust'];

    if (!productIdField || !quantityField || !movementTypeField || !notesField) return null;

    return {
        product_id: productIdField.value,
        variant_option_id: variantOptionIdField ? (variantOptionIdField.value || null) : null,
        quantity_change: parseInt(quantityField.value),
        movement_type: movementTypeField.value,
...
function validateInventoryForm(form, type) {
    ...
    const selectedProduct = productsForInventorySelect.find(p => p.id === productIdField.value);
    ...
    // Check if the selected product actually has variants.
    // The backend response for /products?include_variants=true should ideally indicate this,
    // e.g., via product.weight_options.length > 0 or a specific flag.
    // For now, we rely on the variantSelect being populated and visible.
    if (selectedProduct) {
        const productHasVariants = selectedProduct.variant_count > 0 || selectedProduct.base_price === null; // Or check selectedProduct.weight_options if available
        if (productHasVariants && variantSelect && !variantSelect.value) {
            setFieldError(variantSelect, "Veuillez sélectionner une variante pour ce produit.");
            isValid = false;
        }
    }
...
async function loadInventoryOverview(filterProductId = '') {
    ...
    // --- Performance Note ---
    // The loop below makes an API call for each product (N+1 problem).
    // Ideally, the backend should provide a single endpoint that returns all necessary data for this overview.
    // Example: GET /api/admin/inventory/overview?filterProductId=...
    try {
