...
function getAdminUser() {
    ...
    if (userString) {
        try {
            ...
            clearAdminSession(); // Clear all session data if user data is corrupt
            return null;
        }
    }
    return null;
...
function setAdminUserSession(userData, token) {
    ...
    // updateAdminHeaderDisplay(); // This can be called by admin_main.js
}

function clearAdminSession() {
    sessionStorage.removeItem(ADMIN_TOKEN_KEY);
    sessionStorage.removeItem(ADMIN_USER_KEY);
    sessionStorage.removeItem(PENDING_TOTP_EMAIL_KEY); // Clear pending TOTP email
}
...
function checkAdminLogin() {
    ...
    if (token && adminUser && adminUser.is_admin) {
        ...
        clearAdminSession(); // Ensure session is fully cleared if token/user is invalid
        if (!onLoginPage) {
...
async function handleAdminLoginFormSubmit(event) {
    ...
    if (!email || !password) {
        ...
        if (errorDisplayElement) { errorDisplayElement.textContent = msg; errorDisplayElement.classList.remove('hidden'); }
    ...
    try {
        ...
        if (result.success) {
            if (result.totp_required) {
                // Password correct, TOTP is required
                sessionStorage.setItem(PENDING_TOTP_EMAIL_KEY, email); // Store email for TOTP step
                ...
                // Login successful, no TOTP or TOTP already handled (e.g. by SimpleLogin flow if merged)
            ...
            // Password incorrect or other initial error
            const errorMessage = result.message || 'Invalid email or password.';
            if (errorDisplayElement) { errorDisplayElement.textContent = errorMessage; errorDisplayElement.classList.remove('hidden'); }
...
async function handleTotpVerification() {
    ...
    const errorDisplayElement = document.getElementById('login-error-message'); // Reuse error display
    const submitTotpButton = document.getElementById('submit-totp-button');
    const email = sessionStorage.getItem(PENDING_TOTP_EMAIL_KEY);

    if (errorDisplayElement) { errorDisplayElement.textContent = ''; errorDisplayElement.classList.add('hidden');}
    ...
    if (!email) {
        ...
        // Optionally redirect to login start or refresh page
    ...
    try {
        ...
        if (result.success && result.token && result.user) {
            setAdminUserSession(result.user, result.token);
            sessionStorage.removeItem(PENDING_TOTP_EMAIL_KEY); // Clean up
            if (typeof showAdminToast === 'function') showAdminToast(result.message || 'Login successful!', 'success');
            window.location.href = 'admin_dashboard.html';
        } else {
            ...
            if (errorDisplayElement) { errorDisplayElement.textContent = errorMessage; errorDisplayElement.classList.remove('hidden'); }
            if (typeof showAdminToast === 'function') showAdminToast(errorMessage, 'error');
            totpCodeInput.focus(); // Keep focus on TOTP input for retry
        }
    } catch (error) {
...
// Event listener for SimpleLogin button
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add event listener for TOTP submission if the button exists
    const submitTotpButton = document.getElementById('submit-totp-button');
    if (submitTotpButton) {
        submitTotpButton.addEventListener('click', handleTotpVerification);
    }
     // Add event listener for pressing Enter in TOTP code input
    const totpCodeInput = document.getElementById('admin-totp-code');
    if (totpCodeInput) {
