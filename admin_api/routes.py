...
def get_dashboard_stats():
    ...
    try:
        ...
        # Define pending order statuses
        pending_order_statuses = ('paid', 'processing', 'awaiting_shipment') 
        # Create placeholders for IN clause
        status_placeholders = ','.join(['?'] * len(pending_order_statuses))
        pending_orders_row = query_db(
            f"SELECT COUNT(*) as count FROM orders WHERE status IN ({status_placeholders})",
        ...
        stats = {
            ...
            "pending_orders": pending_orders_row['count'] if pending_orders_row else 0,
            "total_categories": total_categories_row['count'] if total_categories_row else 0,
            "pending_b2b_applications": pending_b2b_applications_row['count'] if pending_b2b_applications_row else 0,
            "success": True
        }
