...
class User(db.Model):
    ...
    password_hash = db.Column(db.String(256)) # Nullable if SSO-only users are allowed
    first_name = db.Column(db.String(80))
    last_name = db.Column(db.String(80))
    role = db.Column(db.String(50), nullable=False, default='b2c_customer', index=True)
    is_active = db.Column(db.Boolean, default=True, nullable=False, index=True)
    is_verified = db.Column(db.Boolean, default=False, nullable=False, index=True) # Email verification
    ...
    # For storing temporary user ID during TOTP verification step
    # This is a simple way; more robust might involve a separate table or cache
    # For this implementation, we'll handle state in the response/session temporarily.

    # SimpleLogin SSO specific fields (optional, but can be useful)
    simplelogin_user_id = db.Column(db.String(255), unique=True, nullable=True, index=True) # Store SimpleLogin's user ID
    ...
    audit_logs = db.relationship('AuditLog', foreign_keys='AuditLog.user_id', backref='acting_user', lazy='dynamic') # Clarified FK
    ...
    def check_password(self, password):
        if not self.password_hash: # User might be SSO only
            return False
        return check_password_hash(self.password_hash, password)

    def generate_totp_secret(self):
        """Generates and sets a new TOTP secret for the user."""
        self.totp_secret = pyotp.random_base32()
        return self.totp_secret

    def get_totp_uri(self, issuer_name=None):
        """
        Generates the provisioning URI for TOTP apps (like Google Authenticator).
        Returns None if totp_secret is not set.
        """
        if not self.totp_secret:
            return None
        
        # Use issuer_name from config if available, otherwise a default
        effective_issuer_name = issuer_name or current_app.config.get('TOTP_ISSUER_NAME', 'Maison Truvra')
        
        return pyotp.totp.TOTP(self.totp_secret).provisioning_uri(
            name=self.email, # User's email as the account name in authenticator app
    ...
    def verify_totp(self, code_attempt):
        ...
        if not self.totp_secret or not self.is_totp_enabled:
            return False # Or raise an error if TOTP is not setup but verification is attempted
        
        totp_instance = pyotp.TOTP(self.totp_secret)
        return totp_instance.verify(code_attempt)

    def to_dict(self): # Basic to_dict for user info, expand as needed
        return {
            "id": self.id,
            "email": self.email,
            ...
            "is_admin": self.role == 'admin' # Convenience flag
        }
...
class Product(db.Model):
    ...
    localizations = db.relationship('ProductLocalization', backref='product', lazy='dynamic', cascade="all, delete-orphan")
    generated_assets = db.relationship('GeneratedAsset', foreign_keys='GeneratedAsset.related_product_id', backref='product_asset_owner', lazy='dynamic')
    def to_dict(self): # Basic representation, expand as needed
        return {
            ...
            "category_name": self.category.name if self.category else None,
             "main_image_url": self.main_image_url, # Relative path
...
class SerializedInventoryItem(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "product_name": self.product.name if self.product else None, # Eager load or handle carefully
...
class StockMovement(db.Model):
    ...
    notes = db.Column(db.Text)
    def to_dict(self): # Basic to_dict for stock movement
        return {
...
class Order(db.Model):
    ...
    items = db.relationship('OrderItem', backref='order', lazy='dynamic', cascade="all, delete-orphan")
    stock_movements = db.relationship('StockMovement', backref='related_order', lazy='dynamic')
    invoice = db.relationship('Invoice', backref=db.backref('order_link', uselist=False)) # Changed backref name to avoid conflict
