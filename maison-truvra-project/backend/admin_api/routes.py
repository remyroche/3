...
from flask_jwt_extended import jwt_required, get_jwt_identity # get_jwt not directly used if g.is_admin is reliable

# Import centralized utilities and services
from ..utils import (
    admin_required, # Use this decorator
...
# Asset service functions are typically called when creating/updating products/items,
# so their results (paths) would be stored in the DB.
...
def get_dashboard_stats():
    ...
    try:
        ...
        total_products = query_db("SELECT COUNT(*) FROM products WHERE is_active = TRUE", db_conn=db, one=True)[0] # Count only active products
        total_orders = query_db("SELECT COUNT(*) FROM orders WHERE status NOT IN ('cancelled', 'pending_payment')", db_conn=db, one=True)[0] # Count relevant orders
        
        # Example: Recent pending B2B applications
        ...
        return jsonify({
            ...
            # Add more stats as needed (e.g., total_revenue, low_stock_items)
        }), 200
...
def create_category():
    ...
    image_file = request.files.get('image_url') # Field name from admin_categories.js
    
    current_user_id = get_jwt_identity() # Admin performing the action
    ...
    try:
        ...
        if image_file and allowed_file(image_file.filename): # Use centralized allowed_file
        ...
        # Return the full URL for the image if created
        image_full_url = url_for('admin_api_bp.serve_asset', asset_relative_path=image_filename_db, _external=True) if image_filename_db else None
        return jsonify(message="Category created successfully", category_id=category_id, slug=slug, image_url=image_filename_db, image_full_url=image_full_url), 201
    ...
    try:
        ...
        for category in categories:
            ...
            if category.get('image_url'):
                 # Use the blueprint's endpoint name for url_for
...
def update_category(category_id):
    ...
    name = data.get('name')
    description = data.get('description') # Can be None if not provided
    parent_id_str = data.get('parent_id')
    ...
    try:
        ...
        if name != current_category['name']: # Check name conflict only if name changed
            if query_db("SELECT id FROM categories WHERE name = ? AND id != ?", [name, category_id], db_conn=db, one=True):
        ...
        if new_slug != current_category['slug']: # Check slug conflict only if slug changed
            if query_db("SELECT id FROM categories WHERE slug = ? AND id != ?", [new_slug, category_id], db_conn=db, one=True):
        ...
        if remove_image and current_category['image_url']:
            ...
            if current_category['image_url']: # Remove old image if new one is uploaded
                full_old_image_path = os.path.join(current_app.config['UPLOAD_FOLDER'], current_category['image_url'])
                if os.path.exists(full_old_image_path):
                    try: os.remove(full_old_image_path)
        ...
        image_full_url_resp = url_for('admin_api_bp.serve_asset', asset_relative_path=image_filename_to_update_db, _external=True) if image_filename_to_update_db else None
        return jsonify(message="Category updated successfully", slug=new_slug, image_url=image_filename_to_update_db, image_full_url=image_full_url_resp), 200
    except sqlite3.IntegrityError as e:
        db.rollback()
...
def delete_category(category_id):
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            return jsonify(message="Category not found during delete operation"), 404 # Should not happen if initial check passes
...
def create_product():
    ...
    try:
        ...
        # Additional details from form (to be stored in description or new fields if schema updated)
        # species = data.get('species') ... etc.
        ...
        # Handle weight_options from form (sent as JSON string)
        weight_options_json_str = data.get('weight_options_json', '[]') # Expecting a field named weight_options_json
        weight_options = []
        try:
            ...
            db.rollback() 
            audit_logger.log_action(user_id=current_user_id, action='create_product_fail', details=f"Invalid weight options format: {e_opt}", status='failure', ip_address=request.remote_addr)
        ...
        if product_type == 'variable_weight' and weight_options:
            for option in weight_options:
                if not all(k in option for k in ('weight_grams', 'price', 'sku_suffix', 'initial_stock')): # initial_stock from form
                ...
                if variant_stock > 0:
                    record_stock_movement(db, product_id, 'initial_stock_variant', quantity_change=variant_stock, variant_id=cursor.lastrowid, reason="Initial stock for new variant", related_user_id=current_user_id)
        ...
        if created_product_data:
            product_dict = dict(created_product_data)
            # Attempt to generate assets and include paths
            try:
                # These functions are from services.asset_service and need current_app context
                # For now, assets are not generated synchronously here to keep it simpler.
                # The frontend expects 'assets' key if available.
                # If assets were generated, their paths would be added to product_dict['assets']
                # e.g., qr_path = generate_qr_code_for_item(f"PROD-{product_id}", product_id, name)
                # passport_path = generate_item_passport(f"PROD-{product_id}", product_id, name)
                # label_path = generate_product_label(product_id, name, description, base_price or 0, currency, sku_prefix)
                # product_dict['assets'] = {
                #     'qr_code_file_path': qr_path, 
                #     'passport_url': url_for('admin_api_bp.serve_asset', asset_relative_path=passport_path, _external=True) if passport_path else None,
                #     'label_file_path': label_path
                # }
                pass # Placeholder for asset generation logic if needed here
            except Exception as asset_e:
                current_app.logger.error(f"Error generating assets for new product {product_id}: {asset_e}")

            response_data["product"] = product_dict
    ...
    try:
        ...
        for product in products:
            ...
            if product['type'] == 'variable_weight' or include_variants_param:
                ...
                # Update aggregate_stock_quantity for display based on sum of variants if it's a variable product
...
@admin_required
def get_product_admin_detail(product_id): # Renamed to avoid conflict with public route if any
    db = get_db_connection()
    try:
        ...
        # Fetch generated assets metadata
        ...
        if assets_data:
            for asset_row in assets_data:
                asset_type_key = asset_row['asset_type'].lower().replace(' ', '_') # e.g., qr_code, passport_html
                # Serve via admin asset route or public passport route
...
def update_product(product_id):
    ...
    try:
        ...
        weight_options_json_str = data.get('weight_options_json', '[]') # Expecting 'weight_options_json'
        weight_options = []
        try:
            weight_options = json.loads(weight_options_json_str) if weight_options_json_str else []
            if not isinstance(weight_options, list): raise ValueError("Weight options must be a list.")
            ...
            db.rollback()
            audit_logger.log_action(user_id=current_user_id, action='update_product_fail_invalid_options', target_type='product', target_id=product_id, details=f"Invalid weight options format: {e_opt}", status='failure', ip_address=request.remote_addr)
            return jsonify(message=f"Invalid format for weight options: {e_opt}"), 400
        ...
        if update_payload_product['type'] == 'variable_weight':
            existing_options_rows = query_db("SELECT id as option_id FROM product_weight_options WHERE product_id = ?", [product_id], db_conn=db) # Use id as option_id
            ...
            for option_data in weight_options:
                ...
                option_id_form = option_data.get('option_id') # This should be product_weight_options.id
                ...
                if option_id_form and int(option_id_form) in existing_option_ids:
                    cursor.execute(
                        """UPDATE product_weight_options SET weight_grams = ?, price = ?, sku_suffix = ?, 
                           ...
                           WHERE id = ? AND product_id = ?""", # Use id as primary key for options
        ...
        updated_product_full_data_response = get_product_admin_detail(product_id) # Use the admin detail getter
        return jsonify(message="Product updated successfully", product=updated_product_full_data_response.get_json().get('product') if updated_product_full_data_response else {}, success=True), 200
...
def delete_product(product_id):
    ...
    try:
        ...
        cursor = db.cursor()
        # Schema has ON DELETE CASCADE for product_images, product_weight_options, stock_movements, reviews.
        ...
        if cursor.rowcount > 0:
            ...
            audit_logger.log_action(user_id=current_user_id, action='delete_product_fail_not_deleted', target_type='product', target_id=product_id, details="Product not found during delete op.", status='failure', ip_address=request.remote_addr)
...
def update_user(user_id):
    ...
    try:
        ...
        rowcount = query_db(f"UPDATE users SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE id = ?", sql_args, db_conn=db, commit=True)
        if rowcount == 0: return jsonify(message="User not found or no changes made"), 404 # Should be caught by above
...
def get_orders():
    db = get_db_connection()
    # Add filters from request.args if needed
    search_filter = request.args.get('search')
    status_filter = request.args.get('status')
    date_filter = request.args.get('date') # Expect YYYY-MM-DD
    ...
    if search_filter:
        conditions.append("(o.id LIKE ? OR u.email LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ?)")
    ...
    try:
        ...
        items_data = query_db(
            """SELECT oi.id as item_id, oi.product_id, oi.product_name, oi.quantity, 
               ...
               FROM order_items oi WHERE oi.order_id = ?""", [order_id], db_conn=db)
...
def update_order_status(order_id):
    ...
    try:
        ...
        if new_status in ['shipped', 'delivered']:
            ...
            # Schema needs 'carrier' or similar like 'shipping_method'
        ...
        query_db(f"UPDATE orders SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE id = ?", tuple(params), db_conn=db, commit=True)
...
def add_order_note(order_id):
    ...
    try:
        ...
        timestamp_str = format_datetime_for_display(None) # Current time
        ...
        query_db("UPDATE orders SET notes_internal = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", (updated_notes, order_id), db_conn=db, commit=True)
    ...
    try:
        for key, value_obj in data.items():
            ...
            if value_to_store is not None:
                ...
                query_db("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)", (key, str(value_to_store)), db_conn=db, commit=True) # Commit each setting or all at end
                updated_keys.append(key)
        # db.commit() # If not committing in query_db
