...
from flask_jwt_extended import jwt_required, get_jwt_identity # For order history
...
# from ..services.email_service import send_email # For order confirmation email
# from ..services.invoice_service import generate_invoice_pdf # If generating invoices on order creation

orders_bp = Blueprint('orders_bp', __name__, url_prefix='/api/orders')

@orders_bp.route('/create', methods=['POST']) # Changed from /checkout to /create for clarity
@jwt_required(optional=True) # Allow guest checkout, but get user_id if logged in
def create_order():
    data = request.get_json()
    current_app.logger.info(f"Order creation request received: {data}")

    audit_logger = current_app.audit_log_service
    db = get_db_connection() # Get DB connection once

    # User identification
    user_id = get_jwt_identity() # Returns None if no valid token
    customer_email = data.get('customer_email') # Frontend should send this for guests or logged-in users
    
    # If logged in, use their verified email if customer_email not provided or different
    if user_id:
        user_db_info = query_db("SELECT email, is_verified FROM users WHERE id = ?", [user_id], db_conn=db, one=True)
        if user_db_info and user_db_info['is_verified'] and (not customer_email or customer_email != user_db_info['email']):
            customer_email = user_db_info['email'] # Prefer verified email of logged-in user
    
    shipping_address_data = data.get('shipping_address') 
    billing_address_data = data.get('billing_address', shipping_address_data) # Default to shipping if not provided
    cart_items_from_payload = data.get('items') # Renamed from cartItems for consistency
    payment_details = data.get('payment_details', {}) # e.g., { method, transaction_id, status }
    ...
    if not billing_address_data or not all(k in billing_address_data for k in required_address_keys):
         # Log if billing is different and incomplete, otherwise it's fine if it defaults to shipping
        if billing_address_data is not shipping_address_data:
    ...
    try:
        ...
        for item_payload in cart_items_from_payload:
            ...
            variant_id = item_payload.get('variant_id') # This should be product_weight_options.id
            quantity_ordered = int(item_payload.get('quantity', 0))
            
            if quantity_ordered <= 0:
                raise ValueError(f"Quantité invalide pour produit ID {product_id}.")

            # Fetch product and variant details to verify price and stock
            product_info = query_db("SELECT name, sku_prefix, type, base_price, aggregate_stock_quantity FROM products WHERE id = ?", [product_id], db_conn=db, one=True)
            if not product_info: raise ValueError(f"Produit ID {product_id} non trouvé.")

            item_price_db = 0
            current_stock_db = 0
            variant_description_db = None
            serialized_item_to_allocate = None # For serialized items

            if product_info['type'] == 'variable_weight' and variant_id:
                variant_info = query_db("SELECT price, aggregate_stock_quantity, weight_grams, sku_suffix FROM product_weight_options WHERE id = ? AND product_id = ?", [variant_id, product_id], db_conn=db, one=True)
                if not variant_info: raise ValueError(f"Variante ID {variant_id} pour produit {product_id} non trouvée.")
            ...
            # Add logic for 'serialized' product type if that's different from 'simple' with stock 1
            # For serialized, stock check might involve finding an 'available' serialized_inventory_item
            # For now, assuming aggregate_stock_quantity is used for simple/variant, and serialized items are handled differently or reduce this aggregate.
            ...
            validated_order_items.append({
                ...
                "serialized_item_id": None, # To be filled if specific serialized item is allocated
                "quantity": quantity_ordered,
                "unit_price": item_price_db,
                "total_price": item_price_db * quantity_ordered,
                "product_name": product_info['name'], # Denormalize
                "variant_description": variant_description_db # Denormalize
            })
        
        # Payment status check (assuming payment_details.status == 'succeeded' for successful payment)
        if payment_details.get('status') != 'succeeded': # Or 'paid' depending on your payment gateway
            ...
            return jsonify(message="Le paiement a échoué ou n'a pas été complété.", success=False), 402 # Payment Required
        ...
        cursor.execute(
            """INSERT INTO orders (user_id, status, total_amount, currency, 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (user_id, 'paid', total_amount_calculated, 'EUR', # Assuming 'paid' after successful payment
        ...
        for item in validated_order_items:
            ...
            # Record stock movement for aggregate stock
            ...
            # If item['serialized_item_id'] is set, update its status to 'sold'
            # if item['serialized_item_id']:
            #    query_db("UPDATE serialized_inventory_items SET status = 'sold', order_item_id = ?, sold_at = CURRENT_TIMESTAMP WHERE id = ?", [cursor.lastrowid, item['serialized_item_id']], db_conn=db, commit=False)
        ...
        # try:
        #     send_email(to_email=customer_email, subject=f"Confirmation de votre commande Maison Trüvra #{order_id}", body_html="<p>Merci pour votre commande...</p>")
        # except Exception as e_mail:
        #     current_app.logger.error(f"Failed to send order confirmation email for order {order_id}: {e_mail}")
    ...
    except sqlite3.Error as dbe: # Catch specific DB errors
        if db: db.rollback()
...
@jwt_required() # User must be logged in to see their order history
...
def get_order_details_public(order_id): # Public facing, so check ownership
    current_user_id = get_jwt_identity()
    db = get_db_connection()
    audit_logger = current_app.audit_log_service
    ...
    try:
        order_data = query_db(
            """SELECT o.*, u.email as customer_email 
               ...
               WHERE o.id = ? AND o.user_id = ?""", # Ensure user owns the order
